...
import { JifelineParser } from './src/lib/trace-parser/jifeline-parser';

// Read Honda trace file
const traceFile = path.join(process.cwd(), 'uploads/traces/1758523181232-dg93e-HONDA_JAZZ_CAM_RYDS.txt');
...
if (b0Ecu) {
  ...
  console.log('  Has Mode 02 (02)?', b0Ecu.discoveredServices.has('02'));

  console.log('\n  Security Levels:', Array.from(b0Ecu.securityLevels));
  console.log('  Session Types:', Array.from(b0Ecu.sessionTypes));
} else {
  console.log('  ECU B0 not found!');
  console.log('  Available ECUs:', Array.from(ecus.keys()));
}